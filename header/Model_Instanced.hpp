...
public:
    ...
    void AddInstanceBuffer(InstanceBufferLayout layout, u_int Buffer)
    {
        glBindVertexArray(VAO);
    ...
    void Draw(Shader &shader, u_int i_count)
    {
        glBindVertexArray(VAO);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);
        shader.Bind();
        ...
        glDrawElementsInstanced(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0, i_count);
    }
