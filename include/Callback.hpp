...
namespace GL
{
    ...
    class CallbackList
    {
        ...
        struct Callback
        {
            ...
            Callback(CallbackId _id, CallbackGroupId _obj_id, const std::function<void()> &func) : id(_id), obj_id(_obj_id), function(func) {}

            ///@brief Moves function
            Callback(CallbackId _id, CallbackGroupId _obj_id, std::function<void()> &&func) : id(_id), obj_id(_obj_id), function(std::move(func)) {}
        ...
        CallbackId current_id = 1; //Keeps track of the function ids, that have been assigned.
    ...
    public:
         ...
         * @return CallbackId a new id
         */
        template <typename F, typename... Args>
        CallbackId Add(F &&function, CallbackGroupId caller_id = 0, Args... args)
        {
         ...
         */
        template <typename F, typename... Args>
        CallbackId Add(const F &function, CallbackGroupId caller_id = 0, Args... args)
        {
            std::lock_guard lock(mutex_queue);
        ...
        void RemoveAll(CallbackGroupId caller_id);
    ...
    class CallbackHandler
    {
        ...
        std::atomic<CallbackGroupId> current_id = 1; //Keeps track of which ids have been assigned.
        std::mutex list_m;
    ...
    public:
        ...
        CallbackGroupId GenId() { return current_id++; }

        ///@brief Calls RemoveAll() on every CallbackList
        void RemoveAll(CallbackGroupId callerid);
